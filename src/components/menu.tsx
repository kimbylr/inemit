        ...
        <ul className="absolute right-0 top-10 bg-white shadow-md w-56 rounded-lg flex flex-col text-gray-35 overflow-hidden">
          ...
          {lists.map(({ id, name, slug, progress }) => (
            <MenuLink
              ...
              noBorder
            >
              <span className="truncate w-full flex gap-2.5 items-center justify-between">
                {name}
                {progress.dueToday > 0 && (
                  <span className="bg-primary-150 text-white text-[12px] flex rounded-full items-center justify-center px-2 py-px font-bold">
                    {progress.dueToday}
                  </span>
                )}
              </span>
